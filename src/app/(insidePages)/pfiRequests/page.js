            ...
            <form className="row">
              ...
              <div className="gap-2 d-md-flex col-12 align-items-center mt-5">
                ...
                <a className="btn btn-outline-primary px-5 py-2 ms-3" onClick={() => setShowFilters(false)}>Cancel</a>
                  {userData?.staffCadre?.includes("admin") &&
                    <button type="button" className="btn btn-secondary px-5 py-2 ms-auto mt-3 mt-md-0" disabled={allPfiRequestsQuery?.isFetching} onClick={() => allPfiRequestsQuery.refetch()}>
                      {allPfiRequestsQuery?.isFetching ? "Fetching..." : "Download As Excel"}
                    </button>}
              </div>
            </form>
          </div>
